...
def signupuser(request):
    if request.method == 'GET':
        ...
        if request.POST['password1'] ==  request.POST['password2']:
            ...
            return render(request, 'todo_app/signupuser.html', {'form': UserCreationForm(), 'error':'Password did not match'})
...
def loginuser(request):
    if request.method == 'GET':
        ...
        if user is None:
            return render(request, 'todo_app/loginuser.html', {'form':AuthenticationForm(), 'error':'Username ans Password did not match'})
        else:
            login(request, user)
            return redirect('currenttodos')
...
def createtodo(request):
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request, 'todo_app/createtodo.html', {'form': TodoForm(), 'error':'Bad data passed in. Try again'})
